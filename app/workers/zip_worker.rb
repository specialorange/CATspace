...
class ZipWorker < Workling::Base
  ...
  def zip_assignment(options)

    assignment = Assignment.find(options[:id]);
    logger.debug "[DEBUG] Inside zip_file"
    logger.debug "[DEBUG] Inside zip_file, id of record is " +
      options[:id].to_s

    source = assignment.path_to_folder
    target = assignment.path_to_attachment(false)

    source.sub!(%r[/$],'')
    logger.debug "[DEBUG] Inside zip_assignment[instructor], source is :" +
      source + " and target is: " + target;
    ...
    target = assignment.path_to_attachment(true)
    logger.debug "[DEBUG] Inside zip_assignment[public], source is :" +
      source + " and target is: " + target;

    #Write the zip file.
    Zip::ZipFile.open(target, 'w') do |zipfile|
          Dir["#{source}/**/**"].each do |file|
            entry = file.sub(source + '/', '')
            if (assignment.is_public?(entry))
              logger.debug { "[DEBUG] zipping file: " + file.to_s }
              zipfile.add(entry, file)
            else
              logger.debug { "[DEBUG] skipping file: " + file.to_s }
            end
          end
        end
